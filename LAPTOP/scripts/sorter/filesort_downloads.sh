...
mv *.mov "$DIR/Movies" 2>/dev/null
mv *.mkv "$DIR/Movies" 2>/dev/null
}
...
scripts()
{
if [ -e "$DIR/scripts" ];then
       echo -n ""
else
       mkdir scripts
fi

mv *.sh "$DIR/scripts" 2>/dev/null
mv *.vim "$DIR/scripts" 2>/dev/null
mv *.python "$DIR/scripts" 2>/dev/null
mv *.py "$DIR/scripts" 2>/dev/null
mv *.pearl "$DIR/scripts" 2>/dev/null
}
