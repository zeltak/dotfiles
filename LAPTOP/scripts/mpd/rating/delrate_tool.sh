...
if [[ $EXTENSION == flac ]]; 
then metaflac --remove-tag=comment "$MUSIC_PATH/$SONG" && metaflac --remove-tag=FMPS_RATING "$MUSIC_PATH/$SONG"

elif [[ $EXTENSION == mp3 ]]; 
then id3v2 --remove-frame "TXXX" "$MUSIC_PATH/$SONG"
