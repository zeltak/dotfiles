...
class CustomApplications(Applications):
	def app_default(self, c):
		...
		if f.extension is not None:
			if f.extension in ('pdf', ):
				c.flags += 'd'
				return self.either(c, 'apvlv', 'zathura')
			if f.extension in ('svg', ):
				c.flags += 'd'
				return self.either(c, 'inkscape', 'feh')
		...
		if f.video or f.audio:
			if f.video:
				c.flags += 'd'
			return self.either(c, 'smplayer', 'totem')
	...
	def app_editor(self, c):
		...
		return self.either(c, 'gvim', 'vim', 'nano')
	...
	def app_smplayer(self, c):
		if c.mode is 1:
			return tup('smplayer', '-fs', *c)

		elif c.mode is 2:
			args = "smplayer -fs -sid 0 -vfm ffmpeg -lavdopts " \
					"lowres=1:fast:skiploopfilter=all:threads=8".split()
			args.extend(c)
			return tup(*args)

		elif c.mode is 3:
			return tup('smplayer', '-mixer', 'software', *c)
...
CustomApplications.generic('opera', 'vim' , 'gvim', 'firefox', 'apvlv', 'evince',
